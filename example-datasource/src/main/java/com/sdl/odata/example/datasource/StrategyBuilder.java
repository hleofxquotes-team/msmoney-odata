 ...
 *
 */
public class StrategyBuilder<T extends MyEntity> {
    ...
    public List<Predicate<T>> buildCriteria(QueryOperation queryOperation, ODataRequestContext requestContext) throws ODataException
    {
    ...
    private void buildFromSelectByKey(SelectByKeyOperation selectByKeyOperation)
    {
        ...
        String entityId = (String) keys.get("id");
        LOG.debug("Selecting by key: {}", entityId);

        predicates.add(e -> e.getId().equalsIgnoreCase(entityId));
    }
    ...
    private void buildFromFilter(CriteriaFilterOperation criteriaFilterOperation)
    {
        ...
        if (criteria instanceof ComparisonCriteria) {
            ...
            // For now we only support here property key/value comparisons, just
            ...
            if (comparisonCriteria.getLeft() instanceof PropertyCriteriaValue
                ...
                Predicate<T> p = entity -> {
                    Object fieldValue = getEntityField(entity, propertyCriteriaValue.getPropertyName());
    ...
    private Object getEntityField(T entity, String propertyName)
    {
        try {
